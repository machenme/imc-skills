...
**Loop 成熟度**：当前实现为 **Closed Loop（闭环）**——每阶段必须通过验证（探活 / SELECT 中文校验）才进入下一阶段，达到生产级交付标准。Review Loop（后台常驻异步审查 Agent 持续反馈）为未来演进方向。
...
> 遵循 Loop Engineering 的核心设计哲学——**机制与策略分离**。底层 mise + 原子工具提供机制（定时器、工作区隔离、运行时安装），本 Contract 定义策略（触发时机、作用范围、预算上限、停止条件、上报通道）。

| 维度 | 约束 |
|------|------|
| **TRIGGER** | 用户传入 `{{root_path}}` 触发 |
| **SCOPE** | 仅限指定目录下的 Java 全栈项目，不扩散至父级或同级目录 |
| **ACTION** | 扫描 → 推断 → 环境对齐 → 数据库注入 → 配置覆写 → 启动 → 探活 → 生成 RUNBOOK |
| **BUDGET** | 每阶段自愈重试 ≤ 3 轮；Maven 构建 ≤ 300s；探活窗口 ≤ 30s；总耗时 ≤ 15 min |
| **STOP** | 全部探活通过 / 任一阶段 3 轮自愈失败 / 输入非法 / 超时 |
| **REPORT** | 里程碑式分层汇报（🟢静默 🟡里程碑 🔴致命）；异常时输出完整故障链 + RUNBOOK.md |
...
* **机制与策略分离**：底层 `mise` + 原子工具 + Worktree 隔离构成机制层（Harness 提供 what），Loop Contract 中的 TRIGGER/SCOPE/BUDGET/STOP 构成策略层（架构师配置 how）。人类的职责是定义策略，不参与执行循环。
...
* **熔断器（Circuit Breaker）**：同一阶段连续 3 轮自愈失败后立即跳闸——停止该阶段所有重试，回退已做变更（恢复 `.bak` 备份），将当前运行栈和日志打包输出为故障报告，转交用户决策。不做无限重试。
* **看门狗（Watchdog）**：若任一 Shell 命令执行超过 300s 无输出（典型场景：国内网络不畅导致 Maven 依赖下载挂起），判定为自旋死锁——强制终止该进程，释放端口占用，进入下一轮重试。防止静默耗尽 Token 预算。
